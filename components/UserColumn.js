...
export default function UserColumn(userInfo) {
  return (
    <tr className='border-dotted border-b-2 border-light-blue-100 x'>
      <td className='w-1/12'>{userInfo.score}</td>
      <td className="text-left w-60 w-2/4"><a href={userInfo.githubURL}><b className='text-gray-800 hover:text-gray-600 '>{userInfo.githubUsername.length > 16 ? `${userInfo.githubUsername.substring(0,16)}..` : userInfo.githubUsername}</b>
